...
def render_timer():
    ...
    timer_text = FONT_SMALL.render(f"Time Left: {remaining_time}s", True, (255, 255, 255))
...
def render_score(zombie_kill_count):
    score_text = FONT_SMALL.render(f"Score: {zombie_kill_count}", True, (255, 255, 255))
    score_rect = score_text.get_rect(center=(screen_width - 1200, screen_height - 1000))
    # Make background darker and larger
    bg_surface = pygame.Surface((score_rect.width + 40, score_rect.height + 20))
    bg_surface.set_alpha(200)  # Increased alpha for better visibility
    bg_surface.fill((0, 0, 0))
...
def main():
    ...
    while running:
        ...
        if not game_over and not win:
            ...
            # Track previous zombie count to calculate kills
            prev_zombie_count = len([z for z in zombies if not z.dead])
            ...
            # Calculate new kills this frame
            current_zombie_count = len([z for z in zombies if not z.dead])
